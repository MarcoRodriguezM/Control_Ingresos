...
public sealed class SolicitudesRepository(IConfiguration configuration) : ISolicitudesRepository
{
    ...
    public async Task<SesionUsuario?> AutenticarAsync(LoginRequest request, CancellationToken cancellationToken)
    {
    ...
    public async Task<IReadOnlyCollection<AprobacionResumen>> ListarAprobacionesAsync(string idUsuarioAprobador, CancellationToken cancellationToken)
    {
        await using var connection = await OpenConnectionAsync(cancellationToken);
    ...
    public async Task<long> DecidirAprobacionAsync(long idSolicitudPersonaArea, DecidirAprobacionRequest request, CancellationToken cancellationToken)
    {
        await using var connection = await OpenConnectionAsync(cancellationToken);
