...
public sealed class CatalogosController(ISolicitudesRepository repository) : ControllerBase
{
    ...
    {
        "estados-generales", "tipos-ingreso", "estados-solicitud", "estados-persona",
        "tipos-documento", "tipos-aplicacion", "areas", "ubicaciones", "categorias-requerimiento",
        "requerimientos"
    };

    [HttpGet("{catalogo}")]
    [ProducesResponseType<IReadOnlyCollection<CatalogoItem>>(StatusCodes.Status200OK)]
    public async Task<ActionResult<IReadOnlyCollection<CatalogoItem>>> Listar(string catalogo, CancellationToken cancellationToken)
    {
        if (!Permitidos.Contains(catalogo))
            return BadRequest(new ProblemDetails { Title = "Catálogo no válido", Detail = $"El catálogo '{catalogo}' no está permitido." });
